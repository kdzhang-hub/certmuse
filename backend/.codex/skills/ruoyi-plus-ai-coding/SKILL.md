...
- `ruoyi-certmuse` 下的后端 Java、Mapper、Service、Controller、BO、VO、权限、查询或 JavaDoc 任务，必须先读通用 [references/backend.md](references/backend.md)，再读专项 [references/certmuse-backend.md](references/certmuse-backend.md)。专项未规定的内容继承通用规则；两者冲突时以 CertMuse 专项规则为准。
- 其他模块的后端 Java、Mapper、Service、Controller、BO、VO、Entity、权限、查询、公共模块或 JavaDoc 任务，先读 [references/backend.md](references/backend.md)。
...
跨模块任务按各自目标模块读取对应 reference。CertMuse 后端任务不得因已读取专项 reference 而跳过通用后端 reference。通用规范提供完整基线，CertMuse 专项只覆盖明确的差异；当前代码用于理解兼容性和复用成熟实现，不得覆盖成文规范。
...
1. 用户在当前任务中的明确要求。
2. 已冻结的 API、数据库和业务契约。
3. 目标文件作用域内适用的 `AGENTS.md`。
4. CertMuse 专项规则（仅 CertMuse 后端任务；其中明确声明的加严规则可以覆盖通用建议）。
5. 当前模块内可复用且不违反上述规则的成熟实现。
6. 通用后端规则与仓库公共基础能力。
7. generator 模板。
8. 通用 Spring Boot / MyBatis-Plus 习惯。

也就是说：

- 同模块已有成熟实现且不违反更高优先级规则时，优先复用该实现。
- 同模块没有现成代码时，再参考 generator 模板。
- 不要因为“更通用”就覆盖掉项目已形成的强约定。

## 后端规则

Java、MyBatis-Plus、BO/VO/entity、controller、mapper、service 的通用完整规则见 [references/backend.md](references/backend.md)。

`ruoyi-certmuse` 后端任务必须在通用规则基础上同时应用 [references/certmuse-backend.md](references/certmuse-backend.md) 中的复杂领域服务、XML Mapper、显式资源可见性、幂等与 Service 接口约定；专项未覆盖的事项继续执行通用规则。
...
`ruoyi-certmuse` 还需先按专项 reference 判断任务属于复杂领域用例还是简单管理 CRUD，再决定是否参考 generator。
