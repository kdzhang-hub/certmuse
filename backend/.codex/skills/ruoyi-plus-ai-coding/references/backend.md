...
本文件描述仓库通用 RuoYi 后端与 generator 风格。目标代码位于 `ruoyi-modules/ruoyi-certmuse` 时，先读取 [certmuse-backend.md](certmuse-backend.md)：

- CertMuse 专项规则优先于本文件的标准 CRUD、`BaseMapperPlus`、`PageQuery`、`@AutoMapper` 和 Service 命名建议。
- 只有专项规则把任务判定为简单管理 CRUD 时，才按本文件和 generator 模板生成标准方法簇。
- 跨模块改动分别遵循各自模块的 reference，不把 CertMuse 的领域模式扩散到 system、demo、workflow 等模块。
