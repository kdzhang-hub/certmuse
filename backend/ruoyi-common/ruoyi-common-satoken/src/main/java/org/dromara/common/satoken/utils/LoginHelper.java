...
public class LoginHelper {
    ...
    public static void login(LoginUser loginUser, SaLoginParameter model) {
        ...
        StpUtil.login(loginUser.getLoginId(),
            model.setExtra(USER_KEY, loginUser.getUserId())
                ...
                .setExtra(ENTRY_TYPE_KEY, loginUser.getEntryType())
        );
    ...
    /**
     * Gets the entry type calculated at login time.
     *
     * @return current entry type, or {@code null} when no valid session exists
     */
    public static EntryType getEntryType() {
        String value = Convert.toStr(getExtra(ENTRY_TYPE_KEY));
    ...
    /**
     * Ensures that the current session belongs to the required entry.
     *
     * @param expected required entry type
     */
    public static void checkEntryType(EntryType expected) {
        if (getEntryType() != expected) {
            throw new ServiceException("没有访问权限", HttpStatus.HTTP_FORBIDDEN);
        }
    }
