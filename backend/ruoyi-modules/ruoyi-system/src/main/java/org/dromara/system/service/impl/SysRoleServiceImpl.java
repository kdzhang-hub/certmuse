...
public class SysRoleServiceImpl implements ISysRoleService, RoleService {
    ...
    public int updateRoleBaseInfo(SysRoleBo bo) {
        ...
        validateAffectedUsersEntry(role.getRoleId());
        return rows;
    }
    ...
    public int updateRoleStatus(Long roleId, String status) {
        ...
        int rows = roleMapper.lambda()
            ...
            .updateCount();
        validateAffectedUsersEntry(roleId);
        return rows;
    }
    ...
    public int deleteAuthUser(SysUserRole userRole) {
        ...
        int rows = userRoleMapper.lambda()
            ...
            .deleteCount();
        if (rows > 0) {
            validateUserEntry(userRole.getUserId());
    ...
    /** Ensures role metadata and role-menu changes do not invalidate linked accounts. */
    private void validateAffectedUsersEntry(Long roleId) {
        if (!roleMapper.selectInvalidEntryUserIds(roleId).isEmpty()) {
            throw new ServiceException("角色变更会导致关联账号入口配置无效");
        }
    }

    private void validateUserEntry(Long userId) {
        List<SysRoleVo> roles = roleMapper.selectRolesByUserId(userId).stream()
            .filter(role -> SystemConstants.NORMAL.equals(role.getStatus()))
            .toList();
        Set<String> permissions = menuMapper.selectMenuPermsByUserId(userId);
        entryPolicy.validateOptional(roles, permissions);
    }
