...
public class SysOssConfigServiceImpl implements ISysOssConfigService {
    ...
    public void init() {
        ...
        for (SysOssConfig config : list) {
            applyRuntimeCredentials(config);
    ...
    /**
     * CertMuse 的本机 MinIO 凭据只从受保护的 systemd runtime.env 读取，
     * 不使用上游初始化 SQL 中的示例凭据，也不将生产密钥写回数据库。
     */
    private void applyRuntimeCredentials(SysOssConfig config) {
        if (!"minio".equals(config.getConfigKey())) {
            return;
        }
        String accessKey = SpringUtils.getProperty("certmuse.oss.minio.access-key");
        String secretKey = SpringUtils.getProperty("certmuse.oss.minio.secret-key");
