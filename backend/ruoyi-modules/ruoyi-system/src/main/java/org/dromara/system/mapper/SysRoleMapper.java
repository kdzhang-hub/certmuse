...
public interface SysRoleMapper extends BaseMapperPlus<SysRole, SysRoleVo>, MPJBaseMapper<SysRole> {
    ...
    default List<SysRoleVo> selectRolesByUserId(Long userId) {
        return this.selectJoinList(SysRoleVo.class, QueryBuilder.lambdaJoin("r", SysRole.class)
            .select(SysRole::getRoleId, SysRole::getRoleName, SysRole::getRoleKey,
                SysRole::getRoleSort, SysRole::getDataScope, SysRole::getStatus, SysRole::getEntryType)
    ...
    /**
     * Finds users affected by one role whose enabled role configuration no longer has a valid entry.
     *
     * @param roleId changed role identifier
     * @return affected user identifiers with an invalid non-empty entry configuration
     */
    List<Long> selectInvalidEntryUserIds(@Param("roleId") Long roleId);
