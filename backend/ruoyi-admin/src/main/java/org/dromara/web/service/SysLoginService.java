...
public class SysLoginService {
    ...
    /**
     * Atomically records the first successful login for an account.
     *
     * @param userId user identifier
     * @return {@code true} when this request recorded the first login
     */
    public boolean markFirstLogin(Long userId) {
        return DataPermissionHelper.ignore(() -> userMapper.lambda()
            .set(SysUser::getFirstLoginAt, LocalDateTime.now())
            .eq(SysUser::getUserId, userId)
            .isNull(SysUser::getFirstLoginAt)
            .updateCount()) == 1;
    }
