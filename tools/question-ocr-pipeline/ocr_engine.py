...
DEFAULT_ENDPOINT = "http://192.168.6.120:8081/layout-parsing"
...
def ocr_chunk(start: int, end: int, pdf: Path, raw_dir: Path, md_dir: Path, endpoint: str) -> None:
...
def main() -> None:
    ...
    for start, end, path in split_pdf(args.source, chunks, args.batch_size):
        for attempt in range(1, 4):
            try:
                ocr_chunk(start, end, path, raw_dir, md_dir, args.endpoint)
                break
