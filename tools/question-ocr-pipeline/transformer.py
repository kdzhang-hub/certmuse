...
WORK = ROOT / "work"
KNOWLEDGE = ROOT / "data/knowledge-points.jsonl"
    ...
    for index, match in enumerate(matches):
        ...
        for offset in range(count):
            ...
            if count > 1:
                question = f"{prompt}\n\n【本题对应原题第{number}空】"
            records.append({
                "qid": f"{QID_PREFIX}-S1-{number:03d}",
...
def parse_cases(section: str) -> list[dict]:
    ...
    for index, match in enumerate(matches):
        ...
        records.append({
            "qid": f"{QID_PREFIX}-S2-{number:02d}",
...
def parse_essays(section: str) -> list[dict]:
    ...
    for index, match in enumerate(matches):
        ...
        records.append({
            "qid": f"{QID_PREFIX}-S3-{number:02d}",
...
def main() -> None:
    global WORK, KNOWLEDGE, SOURCE_NAME, QID_PREFIX
    parser = argparse.ArgumentParser(description="Convert PaddleOCR-VL checkpoints to question-zip/1.0")
    parser.add_argument("work", type=Path)
    parser.add_argument("--knowledge", type=Path, default=KNOWLEDGE)
    parser.add_argument("--source-name", required=True)
    parser.add_argument("--qid-prefix", required=True)
    args = parser.parse_args()
    WORK = args.work.resolve()
    KNOWLEDGE = args.knowledge.resolve()
    SOURCE_NAME = args.source_name
    QID_PREFIX = re.sub(r"[^A-Za-z0-9_-]+", "-", args.qid_prefix).strip("-")
    if not QID_PREFIX:
        raise ValueError("qid-prefix must contain letters or numbers")
    ...
    archive = WORK / f"{output_stem}-question-zip-1.0.zip"
    ...
    report = WORK / f"{output_stem}-复核报告.md"
    report.write_text(
        f"# {output_stem} 复核报告\n\n"
        "## 结果摘要\n\n"
