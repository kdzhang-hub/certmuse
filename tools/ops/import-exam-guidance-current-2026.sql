...
-- CertMuse official exam-guidance data pack, exported 2026-08-20.
-- Prerequisite: all CertMuse migrations through 20260820 are installed.
-- Scope: two draft periods, the current historical 2026-H1 System Architect
-- schedule, and the 34-region long-term directory. It does NOT publish a
-- period and deliberately does NOT create any regional registration record.
--
-- This script refuses to overwrite a published or legacy period. Run it with:
--   psql "$CERTMUSE_DB_URL" -v ON_ERROR_STOP=1 -f tools/ops/import-exam-guidance-current-2026.sql
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM cm_exam_certification WHERE certification_code = 'SYSTEM_ARCHITECT'
    ) OR NOT EXISTS (
        SELECT 1
