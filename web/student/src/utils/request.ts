...
export interface HandledRequestError<T = unknown> extends Error {
  isHandled?: boolean;
  responseData?: T;
}

function createHandledError<T = unknown>(message: string, responseData?: T) {
  const error = new Error(message) as HandledRequestError<T>;
...
export function getHandledRequestError<T = unknown>(error: unknown) {
  return error instanceof Error ? (error as HandledRequestError<T>) : undefined;
}
...
service.interceptors.response.use(
  (res: any) => {
    ...
    if (code === 401) {
      ...
      return Promise.reject(createHandledError(msg, res.data?.data));
  ...
  async (error: any) => {
    ...
    error.isHandled = true;
    error.responseData = error?.response?.data?.data;
    return Promise.reject(error);
  }
);
// 通用下载方法
