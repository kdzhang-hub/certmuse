<template>
  <div class="sidebar-shell" :class="{ 'has-logo': showLogo }" :style="menuStyle">
    ...
    <div class="public-home-area">
      <router-link class="public-home-link" to="/learning/home" title="回到首页" aria-label="回到学习首页">
        <svg-icon icon-class="home" />
        <span v-if="!isCollapse">回到首页</span>
      </router-link>
    </div>
  </div>
</template>
...
const sideTheme = computed(() => (settingsStore.dark ? 'theme-dark' : settingsStore.sideTheme));
...
.public-home-link :deep(.svg-icon) {
  flex: 0 0 16px;
  width: 16px;
  height: 16px;
  margin-right: 10px;
...
.hideSidebar .public-home-link :deep(.svg-icon) {
  margin-right: 0;
}
</style>
