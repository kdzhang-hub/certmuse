<template>
  <div ref="menuContainer" class="topbar-menu-container">
    <el-menu
      class="topbar-menu"
      :ellipsis="false"
...
const visibleRoutes = computed(() => permissionStore.sidebarRouters.filter(route => !route.hidden) as RouteRecordRaw[]);
const topMenus = computed((): RouteRecordRaw[] => {
  return visibleRoutes.value.slice(0, visibleNumber.value);
});
const moreRoutes = computed((): RouteRecordRaw[] => {
  return visibleRoutes.value.slice(visibleNumber.value);
});
function updateVisibleNumber() {
  visibleNumber.value = resolveTopbarVisibleMenuCount(
    visibleRoutes.value.length,
    menuContainer.value?.clientWidth ?? 0
  );
}

useResizeObserver(menuContainer, updateVisibleNumber);

watch(
  () => visibleRoutes.value.length,
  () => nextTick(updateVisibleNumber),
  { immediate: true }
);
...
#app .topbar-menu.el-menu--horizontal > div,
#app .topbar-menu.el-menu--horizontal > .el-sub-menu {
  flex: 0 0 auto;
