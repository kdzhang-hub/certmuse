 ...
 * @param roleIds
 */
export const optionSelect = (roleIds: string[]): AxiosPromise<RoleVO[]> => {
  return request({
 ...
 * 查询角色详细
 */
export const getRole = (roleId: string): AxiosPromise<RoleVO> => {
  return request({
 ...
 * 删除角色
 */
export const delRole = (roleId: string[] | string) => {
  return request({
    url: '/system/role/' + roleId,
    method: 'delete'
  });
...
// 根据角色ID查询部门树结构
export const deptTreeSelect = (roleId: string): AxiosPromise<RoleDeptTree> => {
  return request({
